...
# def oom_watcher(cgroup_ident, mempath, dateformat, uid=0, services=[]):
...
#     efd = libc.eventfd(0,0)
#     try:
#         cgevent_control = os.open("%s/cgroup.event_control" % mempath, os.O_WRONLY) ## $memoryCgroupPath/cgroup.event_control
#         cgoom_control = os.open("%s/memory.oom_control" % mempath, os.O_RDONLY) ##  ^^^^ /memory.oom_control
#     except (IOError, OSError) as e:
#         logger.error("Error creating eventfd for cgroup %s. Unable to monitor for OOMs!. Additional information: %s" % (cgroup_ident, e))
#         thread.exit()
#     writebuf = str(efd)+" "+str(cgoom_control)
#     os.write(cgevent_control, writebuf)
#     os.close(cgevent_control)
...
#         os.read(efd, 8)
#         try:
#             with open("%s/tasks" % mempath, 'r') as tf:
#                 pass
#         except (IOError, OSError):
#             thread.exit()
#         if (time.time() - oldTime >=60):
...
#             fstamp = "%s %s" % (s_mon, s_day)
#             time.sleep(10) # Ran into issues with not finding OOMs in kernel logs, seems to be a delay in being able to read that data out.
#             if uid > 0: ## TODO: add logic for non-user (e.g. service-based) cgroups
#                 for msglog in kernlogs:
...
#                                 logtime = spline[2]
#                                 if logdate == fstamp: ## TODO: It's breaking here. Need to do some magic to get the right filtering scheme for the logfile config.
#                                     fLogtime = datetime.datetime.strptime(" ".join((str(stamp_start.year), logdate, logtime)), dateformat)
#                                     if stamp_start <= fLogtime:
#                                         found.append(line)
...
#                                     finderMsg = "OOMMonitor: Could not find date for OOM. %s needed, found %s." % ( fstamp, logdate )
#                             else:
#                                 finderMsg = "OOMMonitor: Could not find line matching 'Killed' and UID %s" % uid
...
#                 if len(found) == 0 and len(logsBroken) < len(kernlogs):
#                     shouldSend = False
#                     logger.warning("Not sending OOM for %s. Found %d events in logs, and %d broken logs." % (cgroup_ident, len(found), len(logsBroken)))        
#                 else:
#                     shouldSend = True

#                 if shouldSend == True:
#                     oomailer(cgroup_ident)